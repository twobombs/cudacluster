...
RUN apt-get update && apt-get install -y git software-properties-common ant freeglut3-dev openjdk-8-jdk qv4l2 opencl-headers ocl-icd-opencl-dev oclgrind python-setuptools python3-setuptools dkms intel-gpu-tools nvtop && apt-get clean all && apt -y autoremove
