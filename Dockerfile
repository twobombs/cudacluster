...
RUN apt-get update && apt-get install -y git software-properties-common ant freeglut3-dev openjdk-8-jdk qv4l2 ocl-icd-libopencl1 opencl-headers ocl-icd-opencl-dev oclgrind python3-setuptools dkms intel-gpu-tools && apt-get clean all && apt -y autoremove
