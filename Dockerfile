...
RUN add-apt-repository universe && apt-get update && apt-get -y --force-yes upgrade && export DEBIAN_FRONTEND=noninteractive && apt-get install -y libboost-all-dev python-pycuda python-pip && apt-get clean all
